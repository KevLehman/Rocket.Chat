							...
							<FieldsAttachment
								fields={attachment.fields.map((field) => {
									...
									return {
										...rest,
										title: title ? (
											<MarkdownText variant='inline' parseEmoji content={title.replaceAll(/(.*)/g, (line: string) => `${line}  `)} />
										) : null,
										value: value ? (
											<MarkdownText variant='inline' parseEmoji content={value.replaceAll(/(.*)/g, (line: string) => `${line}  `)} />
										) : null,
									};
								})}
							/>
